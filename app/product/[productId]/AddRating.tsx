...
const onSubmit:SubmitHandler<FieldValues> = async(data) =>{
    ...
    if(data.rating==0) {
        setIsLoading(false)
        return toast.error('No rating seleted')
    }
 ...
 if(!user || !product) return null;
     ...
     const deliveredOrder= user?.orders.some(order => order.products.find(item => item.id == product.id) && order.deliverStatus =='delivered')
